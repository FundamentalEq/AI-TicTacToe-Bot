...
class Block() :
    ...
    def utility(self) :
        ...
        if CtMe == 0 :
            ...
            if(self.block[1][1]==Enemy and self.block[2][2]==Enemy):
                ans-=center_value
            if(self.block[1][2]==Enemy and self.block[2][1]==Enemy):
                ans-=center_value
        elif CtEnemy == 0 :
            ans += DiagValue ** CtMe
            if(self.block[1][1]==Me and self.block[2][2]==Me):
                ans+=center_value
            if(self.block[1][2]==Me and self.block[2][1]==Me):
                ans+=center_value
        diag = [self.block[i][3-i] for i in range(4)]
        CtMe = diag.count(Me)
        CtEnemy = diag.count(Enemy)
